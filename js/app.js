...
const App = {
    ...
    init() {
        ...
        try { this.initAiCopilot(); } catch (e) { console.warn(e); }
    },
    ...
    setupViews() {
        const views = ['dashboard', 'symbol-overview', 'option-chain', 'oi-clock', 'discovery', 'sectors', 'pcr-analytics', 'ai-trades'];
        views.forEach(v => {
    ...
    switchView(viewId) {
        ...
        if (viewId === 'ai-trades') this.renderAiTradesView();
    ...
    async renderPcrAnalyticsView(symbolInput) {
        ...
        // Render full-screen chart canvas, snapshots table & AI trade banner
        this.renderPcrAiBanner(cleanSym);
        this.renderPcrAnalyticsChartCanvas(cleanSym);
        this.renderPcrSnapshotsTable(cleanSym);
            ...
            <div class="glass card" style="max-width:520px; width:100%; border:1px solid rgba(255,255,255,0.15); border-radius:12px; padding:1.5rem; box-shadow:0 20px 40px rgba(0,0,0,0.6); position:relative; background:#121826">
                ...
                <div style="background:rgba(255,255,255,0.03); border:1px solid rgba(255,255,255,0.08); border-radius:8px; padding:0.9rem 1rem; margin-bottom:1.25rem">
                    ...
                    <div style="display:flex; justify-content:space-between; margin-bottom:0.2rem; font-size:0.85rem">
                        ...
                        <b class="mono" id="basket-capital-val" style="color:var(--text-bright); font-size:0.95rem">₹${basketMargin.toLocaleString()}</b>
                    </div>
                    <div id="basket-live-badge" style="font-size:0.75rem; color:var(--text-muted); text-align:right; margin-bottom:0.4rem">
                        <i class="fas fa-shield-alt" style="color:var(--primary)"></i> Zerodha SPAN Model
                    </div>
                    <div style="display:flex; justify-content:space-between; margin-bottom:0.4rem; font-size:0.8rem">
                        ...
                        <span id="basket-save-val" style="color:#10b981; font-weight:600">Save ${marginSavedPercent}% (₹${marginSaved.toLocaleString()})</span>
                    </div>
                    <div style="display:flex; justify-content:space-between; font-size:0.85rem; border-top:1px solid rgba(255,255,255,0.05); padding-top:0.4rem; margin-top:0.4rem">
                        ...
                        <b class="mono" id="basket-roi-val" style="color:var(--primary); font-size:1rem">+${roi}%</b>
                    </div>
                </div>
    ...
    updateGeminiStatusUI() {
        ...
        if (apiKey) {
            ...
            if (text) { text.textContent = 'Free Gemini Active'; text.style.color = '#10b981'; }
...
4. Be sharp, disciplined, and speak like an elite proprietary desk trader.`;
    ...
    generateQuantTradeSetup(symbol) {
        ...
        let rawList = [];
        if (this.state.pcrHistory && typeof this.state.pcrHistory === 'object' && !Array.isArray(this.state.pcrHistory)) {
            rawList = this.state.pcrHistory[cleanSym] || [];
        }
        let data = this.sanitize5MinPcrList(rawList);
        ...
        if (data && data.length >= 2) {
            ...
            pcrDelta5m = latest.value - prev1.value;
        ...
        if (cleanSym.includes('BANKNIFTY')) step = 100;
        if (cleanSym.includes('FINNIFTY')) step = 50;
        if (cleanSym.includes('MIDCPNIFTY')) step = 25;
        if (!cleanSym.includes('NIFTY')) {
            if (curSpot < 500) step = 5;
            else if (curSpot < 1500) step = 10;
            else if (curSpot < 3000) step = 20;
            else if (curSpot < 10000) step = 50;
        }
        ...
        const target2 = Math.round(estPremium * 1.70);
        ...
        const catalysts = [
            ...
            `Institutional Flow: ${isBullish ? '🟢 Aggressive Put Writing / Call Unwinding' : '🔴 Heavy Call Writing / Long Unwinding'}`,
            `Strike Anchor: ATM ${atmStrike} | Spot: ₹${curSpot.toFixed(1)}`
        ];
        ...
        return {
            ...
            stopLoss,
            riskReward,
            invalidationSpot: `₹${invalidationSpot.toFixed(1)}`,
            convictionScore,
            curPcr,
            pcrDelta15m,
            spotDelta15m,
    ...
    renderPcrAiBanner(sym) {
        const container = document.getElementById('pcr-ai-trade-banner');
        if (!container) return;
        ...
        container.innerHTML = `
            <div class="pcr-ai-left">
                <div class="pcr-ai-badge ${badgeCls}">
                    <i class="fas ${actionIcon}"></i> ${setup.action} (${setup.strike})
                </div>
                <div class="pcr-ai-details">
                    <span>Est Entry: <b>₹${setup.estPremium}</b></span>
